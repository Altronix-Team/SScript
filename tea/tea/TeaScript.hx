...
/**
	...
	A TeaScript can be a class script or a Haxe Script. 
	
	Once a TeaScript instance is created, it can't switch back to a class or Haxe script.
**/
@:structInit
...
class TeaScript
{
	...
	**/
	...
	/**
		...
		TeaScript.superClassInstances["ExampleClass"] = superClass;
	...
	**/
	...
	/**
		The script to execute. Gets set automatically if you create a `new` TeaScript.
	**/
	...
	**/
	...
	@:noPrivateAccess var scriptX(default, null):XTea;

	/**
		Creates a new TeaScript instance.
	...
	**/
	public function execute():Void
	{
		...
		{
			var expr:Expr = parser.parseString(script, if (scriptFile != null && scriptFile.length > 0) scriptFile else "TeaScript");
			interp.execute(expr);
		}
	}
	...
	**/
	public function set(key:String, obj:Dynamic):TeaScript
	{
	...
	**/
	public function setClass(cl:Class<Dynamic>):TeaScript
	{
		if (cl == null)
		{
			if (traces)
	...
	/**
		...
		@return this instance for chaining.
	**/
	public function setClassString(cl:String):TeaScript
	{
	...
	**/
	...
	/**
		...
		@return Returns this instance for chaining.
	**/
	public function unset(key:String):TeaScript
	{
		if (scriptX != null)
		{
			scriptX.interpEX.variables.remove(key);
			XTea.variables.remove(key);
	...
	public function get(key:String):Dynamic
	{
		if (scriptX != null)
		{
			return
			{
				...
				else if (XTea.variables.exists(key))
					XTea.variables[key];
				else
					null;
			}
	...
	**/
	public function call(func:String, ?args:Array<Dynamic>, ?className:String):TeaCall
	{
			...
			else if (interp == null || !exists(func))
			{
				...
				else
				{
					...
					if (scriptFile != null && scriptFile.length > 1)
						...
						pushException('Function $func does not exist in TeaScript instance.');
				}
			}
			else 
			{
	...
	/**
		...
		@return Returns this instance for chaining.
	**/
	public function clear():TeaScript
	{
		if (scriptX != null)
		{
	...
	**/
	...
	public function doFile(scriptPath:String)
	{
		if (scriptPath != null && scriptPath.length > 0)
			try
				scriptX = new XTea(scriptPath, this)
			catch (e)
			{
	...
	/**
		...
		@return Returns this instance for chaining.
	**/
	public function doString(string:String):TeaScript
	{
		var og:String = "TeaScript";
	...
	**/
	#end
	public static function listScripts(path:String, ?extensions:Array<String>):Array<TeaScript>
	{
		if (!path.endsWith('/'))
			path += '/';
		...
		if (FileSystem.exists(path) && FileSystem.isDirectory(path))
		{
			...
			for (i in files)
			{
				...
				if (hasExtension && FileSystem.exists(path + i))
					list.push(new TeaScript(path + i));
			}
		}
		#elseif openflPos
